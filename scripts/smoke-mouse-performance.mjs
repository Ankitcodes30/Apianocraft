...
export async function runMousePerformanceSmoke(page, check) {
  ...
  await check('Smart mid-session re-centering: deliberate 550ms pause creates new origin', async () => {
    ...
    // Move to (400, 200) (displaced by 200px > 15px) -> pitch bend becomes +1.0
    ...
    const pitchBeforeRecenter = state.pitchBend
    ...
    assert.equal(state.pitchBend, 0, 'Pitch bend initializes to 0 on recenter')
    assert.equal(state.modulation, 0, 'Modulation initializes to 0 on recenter')

    // Move left to (360, 200) (deltaX = -40 relative to new origin 400) -> pitch decreases to -0.20
    await page.evaluate(() => window.__apiano.mousePerfMove(360, 200))
    state = await page.evaluate(() => window.__apiano.mousePerfGetState())
    assert.equal(state.pitchBend, -0.2, 'Pitch bend decreases relative to new origin 400')
  })
  ...
  await check('Zero React re-renders on keyboard during mouse movement', async () => {
    ...
    assert.equal(rendersAfter, rendersBefore, 'Zero React re-renders during mouse performance movement')
  })

  await check('Real DOM mouse dispatch: entry -> recenter on stationary pause -> continuous pitch move', async () => {
    const kbBox = await page.evaluate(() => {
      const el = document.querySelector('.kb')
      if (!el) return null
      const r = el.getBoundingClientRect()
      return { left: r.left, top: r.top, width: r.width, height: r.height }
    })
    assert.ok(kbBox, 'Keyboard container .kb must exist')

    const startX = Math.round(kbBox.left + 100)
    const startY = Math.round(kbBox.top + kbBox.height / 2)

    // Move physical mouse into keyboard
    await page.mouse.move(startX, startY)
    await new Promise((r) => setTimeout(r, 450)) // Wait for 350ms entry dwell

    let state = await page.evaluate(() => window.__apiano.mousePerfGetState())
    assert.ok(state.hasOrigin, 'Real DOM mouse move establishes initial origin after entry dwell')
    const initialOriginX = state.originX

    // Move physical mouse right by 200px
    const targetX = startX + 200
    await page.mouse.move(targetX, startY)

    // Short pause 250ms -> MUST NOT RECENTER
    await new Promise((r) => setTimeout(r, 250))
    state = await page.evaluate(() => window.__apiano.mousePerfGetState())
    assert.equal(state.originX, initialOriginX, 'Real DOM: 250ms pause retains original origin')

    // Stationary pause 400ms more (total 650ms > 550ms with ZERO mouse events) -> RECENTER!
    await new Promise((r) => setTimeout(r, 400))
    state = await page.evaluate(() => window.__apiano.mousePerfGetState())
    assert.ok(state.originX !== initialOriginX, 'Real DOM: 550ms stationary pause recenters origin')
    assert.equal(state.recenterTimerRunning, false, 'Recenter timer completes')

    // Move left by 40px relative to targetX -> pitch decreases
    await page.mouse.move(targetX - 40, startY)
    state = await page.evaluate(() => window.__apiano.mousePerfGetState())
    assert.ok(state.pitchBend < 1.0, 'Real DOM: pitch responds relative to new origin')

    // Move outside keyboard
    await page.mouse.move(10, 10)
    await new Promise((r) => setTimeout(r, 100))
    state = await page.evaluate(() => window.__apiano.mousePerfGetState())
    assert.equal(state.active, false, 'Real DOM: mouse leave resets mouse performance')
  })
