...
export class ToneChain {
  ...
  constructor(private readonly ctx: BaseAudioContext) {
    ...
    // Volume — level, automated smoothly. Stereo 2-channel up-mixing for true panning.
    this.volumeGain = ctx.createGain()
    this.volumeGain.gain.value = 1
    this.volumeGain.channelCount = 2
    this.volumeGain.channelCountMode = 'explicit'
    ...
    // Reverb routing: post-chorus -> reverbSend -> convolver -> reverbWet -> output
    chorusOut.connect(this.reverbSend)
    this.reverbSend.connect(this.convolver)
    this.convolver.connect(this.reverbWet)
    this.reverbWet.connect(this.output)

    // Delay routing: post-chorus -> delaySend -> delayNode -> delayWet -> output
    chorusOut.connect(this.delaySend)
  ...
  // ------------------------------------------------------------------ envelope
  readonly getAttack = (): number => this.attackTime
  readonly getRelease = (): number => this.releaseTime

  /** Seconds, 0.001..2.0. */
  readonly setAttack = (seconds: number): void => {
    this.attackTime = clampFinite(seconds, 0.001, 2.0)
  }

  /** Seconds, 0.01..5.0. */
  readonly setRelease = (seconds: number): void => {
    this.releaseTime = clampFinite(seconds, 0.01, 5.0)
  }
  ...
  readonly setReverbAmount = (value: number): void => {
    ...
    this.reverbConnected = v > 0
    this.reverbSend.gain.setTargetAtTime(v, this.ctx.currentTime, TAU_S)
    this.reverbWet.gain.setTargetAtTime(v * 1.2, this.ctx.currentTime, TAU_S)
  }
  ...
  readonly setDelayAmount = (value: number): void => {
    ...
    this.delayConnected = v > 0
    this.delaySend.gain.setTargetAtTime(v, this.ctx.currentTime, TAU_S)
    this.delayWet.gain.setTargetAtTime(v * 1.2, this.ctx.currentTime, TAU_S)
  }
