...
import { HarmoniumClassicInstrument } from './instruments/HarmoniumClassicInstrument'
import { HarmoniumCouplerInstrument } from './instruments/HarmoniumCouplerInstrument'
import { HarmoniumDroneInstrument } from './instruments/HarmoniumDroneInstrument'
import { FluteInstrument } from './instruments/FluteInstrument'
import { ViolinInstrument } from './instruments/ViolinInstrument'
import { PiccoloInstrument } from './instruments/PiccoloInstrument'
import { BanjoInstrument } from './instruments/BanjoInstrument'
...
export class AudioEngine {
  ...
  // Harmonium Harmony state
  private harmoniumCoupler = false
  private harmoniumDrone: number | null = null

  get harmoniumCouplerEnabled(): boolean {
    return this.harmoniumCoupler
  }

  setHarmoniumCoupler(enabled: boolean): void {
    this.harmoniumCoupler = enabled
    this.emit({ type: 'tuning' })
  }

  get harmoniumDroneNote(): number | null {
    return this.harmoniumDrone
  }

  setHarmoniumDrone(midiNote: number | null): void {
    if (this.harmoniumDrone !== null && this.voiceManager) {
      const effectivePrev = this.applyTuning(this.harmoniumDrone)
      this.voiceManager.noteOff(effectivePrev, false)
    }
    this.harmoniumDrone = midiNote
    if (midiNote !== null && this.voiceManager && this.ctx) {
      const seq = ++this.eventSeq
      const effectiveDrone = this.applyTuning(midiNote)
      void this.spawnVoice(effectiveDrone, 0.7, 'mouse', seq, 'main')
    }
    this.emit({ type: 'tuning' })
  }
  ...
  private async doCreate(): Promise<void> {
    ...
    this.bank.register(new HarmoniumClassicInstrument())
    this.bank.register(new HarmoniumCouplerInstrument())
    this.bank.register(new HarmoniumDroneInstrument())
    this.bank.register(new FluteInstrument())
    this.bank.register(new ViolinInstrument())
    this.bank.register(new PiccoloInstrument())
    this.bank.register(new BanjoInstrument())
  ...
  setMainToneAttack(seconds: number): void {
    const s = clampFinite(seconds, 0.001, 2.0)
    this.mainToneTargets.attackTime = s
    this.mainTone?.setAttack(s)
  }

  setMainToneRelease(seconds: number): void {
    const s = clampFinite(seconds, 0.01, 5.0)
    this.mainToneTargets.releaseTime = s
    this.mainTone?.setRelease(s)
  }
  ...
  private applyMainToneTargets(): void {
    ...
    c.setPan(t.pan)
    if (t.attackTime !== undefined) c.setAttack(t.attackTime)
    if (t.releaseTime !== undefined) c.setRelease(t.releaseTime)
  ...
  setDualToneAttack(seconds: number): void {
    const s = clampFinite(seconds, 0.001, 2.0)
    this.dualToneTargets.attackTime = s
    this.dualTone?.setAttack(s)
  }

  setDualToneRelease(seconds: number): void {
    const s = clampFinite(seconds, 0.01, 5.0)
    this.dualToneTargets.releaseTime = s
    this.dualTone?.setRelease(s)
  }
  ...
  setLowerToneAttack(seconds: number): void {
    const s = clampFinite(seconds, 0.001, 2.0)
    this.lowerToneTargets.attackTime = s
    this.lowerTone?.setAttack(s)
  }

  setLowerToneRelease(seconds: number): void {
    const s = clampFinite(seconds, 0.01, 5.0)
    this.lowerToneTargets.releaseTime = s
    this.lowerTone?.setRelease(s)
  }
  ...
  noteOn(req: NoteOnRequest): void {
    ...
    if (this.splitEnabled && req.note < this.splitPoint) {
      ...
      if (this.harmoniumCoupler && req.note + 12 <= 127) {
        const couplerNote = this.applyTuning(req.note + 12)
        this.voiceManager.retrigger(couplerNote, this.envConfig.release * 0.25)
        void this.spawnVoice(couplerNote, velocity * 0.85, req.source, seq, 'main', portamentoFromRate)
      }
  ...
  noteOff(req: { note: number }): void {
    ...
    if (this.splitEnabled && req.note < this.splitPoint) {
      const effectiveLower = this.applyLowerTuning(req.note)
      const relSec = this.lowerTone?.getRelease() ?? this.envConfig.release
      const releasedLower = this.voiceManager.noteOff(effectiveLower, this.sustainPedal, relSec)
      ...
      const effectiveMain = this.applyTuning(req.note)
      const mainRelSec = this.mainTone?.getRelease() ?? this.envConfig.release
      const releasedMain = this.voiceManager.noteOff(effectiveMain, this.sustainPedal, mainRelSec)
      ...
      if (this.harmoniumCoupler && req.note + 12 <= 127) {
        const couplerNote = this.applyTuning(req.note + 12)
        this.voiceManager.noteOff(couplerNote, this.sustainPedal, mainRelSec)
      }

      if (this.dualEnabled) {
        const effectiveDual = this.applyDualTuning(req.note)
        const dualRelSec = this.dualTone?.getRelease() ?? this.envConfig.release
        const releasedDual = this.voiceManager.noteOff(effectiveDual, this.sustainPedal, dualRelSec)
